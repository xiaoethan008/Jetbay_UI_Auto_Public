...
def test_submit_round_trip_proposal(home_page, page):
    """提交往返方案。"""
    first_name = "Codex"
    last_name = "Tester"
    email = f"codex.roundtrip+{datetime.now().strftime('%Y%m%d%H%M%S')}@example.com"
    phone_number = "1234567890"
    message = "Automated round-trip charter proposal submission."

    results_page = SearchResultsPage(page)
    for _ in range(6):
        home_page.prepare_searchable_city_pair(trip_type_text=HomePageLocators.ROUND_TRIP_TEXT)
        ...
        raise AssertionError("Unable to reach the round-trip search results page with random searchable cities.")
